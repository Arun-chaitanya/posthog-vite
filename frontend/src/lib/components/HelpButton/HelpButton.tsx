...
import {
  DefaultAction,
  inAppPromptLogic,
} from "lib/logic/inAppPrompt/inAppPromptLogic";
...
export function HelpButton({
  ...
  const { toggleHelp, hideHelp } = useActions(
    helpButtonLogic({ key: customKey })
  );
  ...
  return (
    <>
        ...
        items={[
          ...
          !contactOnly && {
            items: [
              ...
              {
                label: `${
                  hedgehogModeEnabled ? "Disable" : "Enable"
                } hedgehog mode`,
                icon: <IconFlare />,
                onClick: () => {
      ...
      >
        <div
          className={clsx("help-button", inline && "inline")}
          onClick={toggleHelp}
          data-attr="help-button"
        >
          {customComponent || (
            <>
              <IconHelpOutline />
